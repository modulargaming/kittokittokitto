...
for ( $lvl = 1; $lvl <= 14; $lvl += 1) {
	if ($CITY['stone_break'] == $lvl)
	{
		$wood_cost = $CITY_CONFIG["stonebreak_wood_cost_lvl_$lvl"];
		$clay_cost = $CITY_CONFIG["stonebreak_clay_cost_lvl_$lvl"];
		$stone_cost = $CITY_CONFIG["stonebreak_stone_cost_lvl_$lvl"];
		$iron_cost = $CITY_CONFIG["stonebreak_iron_cost_lvl_$lvl"];
		$gold_cost = $CITY_CONFIG["stonebreak_gold_cost_lvl_$lvl"];
		$take_time = $CITY_CONFIG["stonebreak_upgrade_lvl_$lvl"];
	}
}

if ($CITY['stone_break'] == '15')
{ 
	echo "<p>You have reached the max limit on Stone Break.</p>";
}
else 
{
	switch($_REQUEST['state'])
	{
		default:
			if ($CITY['btime'] < date("Y-m-d H:i:s")) 
			{
				...
				$renderer->assign('building','a Stone Break');
				$renderer->assign('id','stonebreak');
			...
			$ERRORS = array();
			   ...
			   if($stone_cost > $CITY['stone'])
            	...
            	else
            	{
                	...
                	$min = 'minutes';
					$city->setB_building('stonebreak');
					$city->setB_time(date("Y-m-d H:i:s", strtotime(+$take_time . $min)));
					$city->setB_lvl($CITY['stone_break'] + 1);
					$city->save();
					redirect('city');
            	}
			}
