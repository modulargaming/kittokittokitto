...
for ( $lvl = 1; $lvl <= 14; $lvl += 1) {
	if ($CITY['clay_mine'] == $lvl)
	{
		$wood_cost = $CITY_CONFIG["claymine_wood_cost_lvl_$lvl"];
		$clay_cost = $CITY_CONFIG["claymine_clay_cost_lvl_$lvl"];
		$stone_cost = $CITY_CONFIG["claymine_stone_cost_lvl_$lvl"];
		$iron_cost = $CITY_CONFIG["claymine_iron_cost_lvl_$lvl"];
		$gold_cost = $CITY_CONFIG["claymine_gold_cost_lvl_$lvl"];
		$take_time = $CITY_CONFIG["claymine_upgrade_lvl_$lvl"];
	}
}

if ($CITY['clay_mine'] == '15')
{ 
	echo "<p>You have reached the max limit on Clay Mine.</p>";
}
else 
{
	switch($_REQUEST['state'])
	{
		default:
			if ($CITY['btime'] < date("Y-m-d H:i:s")) 
			{
				...
				$renderer->assign('building','a Clay Mine');
				$renderer->assign('id','claymine');
		...
		case 'process':
			if ($CITY['btime'] < date("Y-m-d H:i:s"))
			{
	            ...
	            else
	            {
					...
					$min = 'minutes';
					$city->setB_building('claymine');
					$city->setB_time(date("Y-m-d H:i:s", strtotime(+$take_time . $min)));
					$city->setB_lvl($CITY['clay_mine'] + 1);
					$city->save();
					redirect('city');
	            }
			}
