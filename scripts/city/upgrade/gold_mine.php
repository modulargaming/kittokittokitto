...
for ( $lvl = 1; $lvl <= 14; $lvl += 1) {
	if ($CITY['gold_mine'] == $lvl)
	{
		$wood_cost = $CITY_CONFIG["goldmine_wood_cost_lvl_$lvl"];
		$clay_cost = $CITY_CONFIG["goldmine_clay_cost_lvl_$lvl"];
		$stone_cost = $CITY_CONFIG["goldmine_stone_cost_lvl_$lvl"];
		$iron_cost = $CITY_CONFIG["goldmine_iron_cost_lvl_$lvl"];
		$gold_cost = $CITY_CONFIG["goldmine_gold_cost_lvl_$lvl"];
		$take_time = $CITY_CONFIG["goldmine_upgrade_lvl_$lvl"];
	}
}

if ($CITY['gold_mine'] == '15')
{ 
	echo "<p>You have reached the max limit on Gold Mine.</p>";
}
else 
{
	switch($_REQUEST['state'])
	{
		default:
			if ($CITY['btime'] < date("Y-m-d H:i:s")) 
			{
				...
				$renderer->assign('building','a Gold Mine');
				$renderer->assign('id','goldmine');
		...
		case 'process':
			if ($CITY['btime'] < date("Y-m-d H:i:s"))
			{
				...
				else
            	{
                	...
                	$min = 'minutes';
					$city->setB_building('goldmine');
					$city->setB_time(date("Y-m-d H:i:s", strtotime(+$take_time . $min)));
					$city->setB_lvl($CITY['gold_mine'] + 1);
					$city->save();
					redirect('city');
            	}
			}
