...
<div align='center'>	
    <table class='inputTable' width='50%' style='margin-left: auto; margin-right: auto;'>
        ...
        <tr>
            <td class='inputTableRowAlt inputTableSubhead'>Refered</td>
            <td class='inputTableRowAlt'>{$profile.ref} {if $profile.ref < 2}User{else}Users{/if}</td>
        </tr>
        {if $profile.refby != null}
        <tr>
            <td class='inputTableRowAlt inputTableSubhead'>Refered By</td>
            <td class='inputTableRowAlt'>{$profile.refby}</td>
        </tr>
        {/if}
        <tr>
            <td style='text-align: center;' class='inputTableRow' colspan='2'>
                <form action='{$display_settings.public_dir}/write-message/{$profile.id}/' method='get' style='display: inline; padding-right: 1em;'>
...
<div align='center' style='margin-top: 3em;'>
    ...
    <div class='pet-box'>
        <div align='center'>
            ...
            <p><strong>{mgurl link_text=$pet.name slug='pet' args=$pet.id}</strong></p>
        </div>
    </div>
    {/section}
    <br clear='all' />
