...
<p align='center'>{mgurl link_text='Compose New Message' slug='write-message'}</p>
...
<div align='center'>
    <form action='{$display_settings.public_dir}/messages' method='post' onSubmit='return confirm("Are you sure you wish to delete the selected messages?")'>
        ...
        <table class='dataTable' width='70%'>
            ...
            <tr>
                <td class='{$class}' align='center'{if $message.read == 'N'} style='font-weight: bold;'{/if}>{mgurl link_text=$message.title slug='message' args=$message.id}</td>
                <td class='{$class}' align='center'>{mgurl link_text=$message.sender.name slug='profile' args=$message.sender.id}</td>
