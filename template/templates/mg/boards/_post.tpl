<div id=post_{$post.id} class='board-post'>
    <div class='post-userinfo'>
        <ul>
            <li class='username'>{mgurl link_text=$post.username slug='profile' args=$post.user_id}</li>
    ...
    <div class='post-content'>
        <p class='post-content-header'>Posted at {$post.posted_at} &mdash; {mgurl link_text='Link' slug='thread' args=`$thread.id`/`$page`#`$post.id` name=$post.id}{if $post.can_edit == 1} &mdash; {mgurl link_text='Edit' slug='edit-post' args=$post.id}{/if}</p>
