<div id='breadcrumb-trail'>{mgurl link_text='Boards' slug='boards'} &raquo; {$board.category} &raquo; {$board.name}</div>
...
<div align='center'>
    <table class='dataTable' width='85%'>
        ...
        <tr>
            <td class='{$class}' align='center'>{if $thread.locked == 'Y'}<img src='{$display_settings.public_dir}/resources/images/lock.png' alt='(L)' border='0' /> {/if}{if $thread.sticky == 1}Sticky: {/if}{mgurl link_text=$thread.topic slug='thread' args=$thread.id}{if $thread.last_page > 1} <small>({mgurl link_text='Last Page' slug='thread' args=`$thread.id`/`$thread.last_page`})</small>{/if}</td>
            <td class='{$class}' align='center'>{mgurl link_text=$thread.poster_username slug='profile' args=$thread.poster_id}</td>
